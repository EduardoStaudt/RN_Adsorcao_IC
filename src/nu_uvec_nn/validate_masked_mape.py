# -*- coding: utf-8 -*-
"""Validação NU -> U_vec usando MAPE mascarado + RMSE/MAE/R2."""
...
def mape_masked(y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 1e-6) -> tuple[float, int, int]:
...
def run_validation(
    ...
    print(f"[INFO] split={split} | threshold={threshold}")
    ...
    mape, used, total = mape_masked(y_true, y_pred, threshold=threshold)
    ...
    metrics_global = {
        ...
        "acc_%": acc,
        "mape_used": int(used),
        "mape_total": int(total),
        "threshold": float(threshold),
    }

    print("\n================= MÉTRICAS GLOBAIS (masked) =================")
    print(f"MSE={mse:.6g} RMSE={rmse:.6g} MAE={mae:.6g} R2={r2:.6g} "
          f"MAPE_masked={mape:.3f}% (used={used}/{total}) ACC={acc:.2f}%")
    print("=============================================================\n")
    ...
    for j, name in enumerate(out_cols):
        ...
        mape_j, used_j, total_j = mape_masked(yt, yp, threshold=threshold)
        acc_j = acc_from_mape(mape_j)
        rows.append({
            "variavel": name,
            "mse": mse_j, "rmse": rmse_j, "mae": mae_j, "r2": r2_j,
            "mape_%": mape_j, "acc_%": acc_j,
            "mape_used": used_j, "mape_total": total_j,
        })
    ...
    def plot_sample(i: int) -> None:
        ...
        plt.title(f"U_vec | amostra={i} | split={split} | masked")
        plt.xlabel("k (0..24)")
        plt.ylabel("u_k")
        plt.legend()
        plt.tight_layout()
    ...
    (outdir / "run_meta.json").write_text(json.dumps({
        "tag": tag, "method": "masked", "split": split,
        "model": str(model_path), "dataset_npz": str(dataset_npz),
        "threshold": threshold,
...
def main() -> None:
    ...
    outdir = cfg.NU_OUT_INFER / tag / "masked"

    run_validation(
        tag=tag, split=a.split,
        model_path=Path(a.model), dataset_npz=Path(a.dataset_npz),
        outdir=outdir, threshold=a.threshold, n_show=a.n_show, seed=a.seed
    )
