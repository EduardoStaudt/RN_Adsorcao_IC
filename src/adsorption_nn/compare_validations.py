...
def compare(tag: str, out_base: Path) -> tuple[pd.DataFrame, str]:
    ...
    # sem ACC
    metrics = [c for c in ["mse", "rmse", "mae", "mape_%", "r2", "mape_used", "mape_total"] if c in df_long.columns]
    ...
    for m in ["mse", "rmse", "mae", "mape_%", "r2"]:
        c_eps = f"{m}_EPS"
        c_msk = f"{m}_MASKED"
    ...
    summary_lines.append("- O MASKED mantém MAPE mais interpretável, pois ignora |y_true| < threshold.")
    summary_lines.append("=" * 72)
