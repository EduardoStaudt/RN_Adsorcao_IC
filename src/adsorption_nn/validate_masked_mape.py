# -*- coding: utf-8 -*-
"""
Validação / inferência do modelo de ADSORÇÃO (22 -> 208) usando MAPE_MASKED.

MAPE_MASKED:
ignora pontos onde |y_true| < threshold (evita explosão por zeros)
...
def mape_masked(y_true, y_pred, threshold=1e-3):
    """
    MAPE mascarado:
    usa apenas pontos onde |y_true| >= threshold
    """
...
def compute_metrics_masked(y_true, y_pred, threshold=1e-3):
    ...
    mape_v, n_used, n_total = mape_masked(y_true, y_pred, threshold=threshold)
...
parser.add_argument("--mape_threshold", type=float, default=1e-3)
...
print("[DEBUG] METHOD      = MASKED MAPE")
print("[DEBUG] THRESHOLD   =", args.mape_threshold)
...
if "data" not in data.files or "columns" not in data.files:
    raise ValueError("Esse validate_masked_mape.py espera NPZ com chaves: data e columns.")
...
# Métricas globais
# =======================================================
glob = compute_metrics_masked(y_true, y_pred, threshold=args.mape_threshold)
print(
    f"[GLOBAL][MASKED] RMSE={fmt(glob['rmse'])}  MAE={fmt(glob['mae'])}  "
    f"MAPE_mask={fmt(glob['mape_%'])}%  R2={fmt(glob['r2'])}  "
    f"(mape_used={glob['mape_used']}/{glob['mape_total']})"
)
...
for name, s in [
    ...
    m = compute_metrics_masked(y_true[:, s], y_pred[:, s], threshold=args.mape_threshold)
...
for j, name in enumerate(FINAL_COLS):
    yt = y_true[:, j]
    yp = y_pred[:, j]
    m = compute_metrics_masked(yt, yp, threshold=args.mape_threshold)
